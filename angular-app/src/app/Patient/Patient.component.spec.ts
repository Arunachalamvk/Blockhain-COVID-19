...
describe('PatientComponent', () => {
  ...
  it('should update the table when a Patient is added', fakeAsync(() => {
    ...
    sinon.stub(component.servicePatient, 'addAsset').returns(new Observable<any>(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Patient is updated', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.servicePatient, 'updateAsset').returns(new Observable<any>(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Patient is deleted', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.servicePatient, 'deleteAsset').returns(new Observable<any>(observer => {
      observer.next('');
      observer.complete();
    }));
